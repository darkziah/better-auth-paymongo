...
The `paymongo()` plugin registers three HTTP endpoints under `/api/auth/paymongo/`.
...
Checks if a feature is accessible. Verifies payment status (cached 60s) and handles period rollover.
